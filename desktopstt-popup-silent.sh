...
# Create a temporary Python script that uses popup_recorder with auto-close
cat > /tmp/popup_recorder_auto_close.py << 'EOF'
#!/usr/bin/env python3
"""
Modified popup recorder that automatically closes after transcription.
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Auto-closing popup recorder')
...
PYTHONWARNINGS=ignore .venv/bin/python /tmp/popup_recorder_auto_close.py --output "$TEMP_OUTPUT" --silence-threshold 0.05 --silence-duration 3.0 2>/dev/null
