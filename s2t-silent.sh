...
LOG_FILE="/tmp/s2t-silent-debug.log"
echo "Starting s2t-silent.sh at $(date)" > "$LOG_FILE"
...
# Run the headless recorder and capture its output
PYTHONWARNINGS=ignore "$S2T_PATH/.venv/bin/python" -m s2t.headless_recorder --silence-threshold 0.05 --silence-duration 3.0 --debug > "$OUTPUT_FILE" 2>> "$LOG_FILE"
PYTHON_EXIT_CODE=$?
...
if [ -f "$(dirname "$0")/s2t/truly_silent.py" ]; then
